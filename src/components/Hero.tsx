...
const Hero = () => {
  ...
  // Typing effect
  useEffect(() => {
    iRef.current = 0; // reset index
    typingRef.current = true;
    setDisplayedText("");
    const type = () => {
      if (typingRef.current) {
        setDisplayedText((prev) => {
          const nextText = prev + fullText.charAt(iRef.current);
          return nextText;
        });
        iRef.current++;
        if (iRef.current === fullText.length) {
          ...
          timeoutRef.current = setTimeout(() => {
            ...
            iRef.current = -1;
            setDisplayedText("");
            timeoutRef.current = setTimeout(type, 600); // pause before restarting
          }, 1000); // pause after finishing
          return;
        }
  ...
  // Handler to trigger download via JavaScript
  ...
  return (
    <section
      ...
      style={{
        backgroundImage:
          "url('https://images.unsplash.com/photo-1518932945647-7a1c969f8be2?auto=format&fit=crop&q=80&w=1600')",
      }}
    >
      ...
      <div className="container mx-auto px-4 py-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          >
            ...
            <div className="inline-block ml-2 mb-4 py-1 px-3 rounded-full bg-primary/20 border border-primary/30">
              <span className="text-primary font-medium">Data Analyst</span>
            </div>
            ...
            <div className="overflow-hidden h-14 mb-6">
              <div className="font-medium text-xl md:text-2xl text-white/90">
                {displayedText}
            ...
            <div className="flex flex-wrap gap-4">
              <Button size="lg" onClick={handleResumeDownload} className="group">
          ...
          <div
            ref={imageRef}
            className="flex justify-center md:justify-center opacity-0 transition-all duration-700"
            style={{
          ...
          >
            ...
            <div className="absolute top-5 left-1/4 animate-float bg-card p-2 rounded-lg shadow-lg border border-border z-20 hidden md:block">
              <span className="text-sm font-medium flex items-center gap-1">
                <span className="w-2 h-2 bg-dsblue rounded-full"></span> Machine Learning
              </span>
            </div>

            <div className="absolute bottom-3 right-1/4 animate-float-delayed bg-card p-2 rounded-lg shadow-lg border border-border z-20 hidden md:block">
              <span className="text-sm font-medium flex items-center gap-1">
